...
def syndrome(message):
    ...
    for ligne_i in range(l_1):
        add_ligne = []
        for colonne_j in range(l_1+1):
            add_ligne.append(s[ligne_i+colonne_j])
        s_ext.SetRow(ligne_i, add_ligne)
        ...
        taille = l_1 + 1
        for ligne_i in range(1, taille):
            a = 0
            for colonne_j in range(1, len(b) + 1):
                a = F.Add(a, F.Multiply(u[taille - ligne_i - 1, taille - colonne_j], b[-colonne_j]))
            if a == 0:
                b = [a] + b
            else:
                a = F.Divide(a, u[taille - ligne_i - 1, taille - ligne_i - 1])
                b = [a] + b
    indice = []
    b.reverse()
    for ligne_i in range(0, 16):  # on test pour chaque élément non nul du corps
        evaluation_q1 = 0
        for l in range(l_1+1):  # pour chaque monome
            e = evaluation_q1
            evaluation_q1 = F.Add(e, F.Multiply(b[l], puissance(X[ligne_i], l_1-l)))
        if evaluation_q1 == 0:
            indice.append(ligne_i-1)
    t = len(indice)
    ...
    I = H_ext.Solve(S_sol)
    E = [0]*15
    for ligne_i in range(len(S_sol)):
        E[indice[ligne_i]] = I[ligne_i]
